...
> 故名思意，指针数组即**存储指针的数组**，该数组中所有的元素都是指针变量。

- ```c++
    char* arr[3] = {"dream", "it", "possible"}
    // 由于 * 的优先级低于 []
    // 因此 arr 先与 [] 结合，此时 arr[]是一个数组
    // 然后与 char* 结合，说明该数组中的变量为 char* 类型
    ```

### 1.4 数组指针

> 数组指针，在理解是最好记住添加一个的，读作**数组的指针**，可以很方便的与指针数组进行区别。

**有 3 种方式可以定义一个数组指针**

- ```
    // method01:定义数组类型，根据数组类型定义数组指针
    
    int arr[5] = {1, 2, 3, 4, 5};
    // 定义数组类型:MYARR[5]
    typedfe int MYARR[5];
    // 根据数组类型，定义数组指针
    // 因为一维数组名是 数组首元素的地址；对数组名进行取地址则是 数组的首地址
    // 指针 pArr 指向数组 arr
    MYARR* pArr = &arr;
    ...
    {
    	// 因为 pArr = &a;所以 (*pArr) == a; 所以 (*pArr)[0] 是 arr 第 0 个元素
    	std::cout << (*pArr)[i] << '\t';
    }
    std::cout << std::endl;
    ```
...
- ```c++
    // method02:定义数组指针类型，直接通过数组指针类型
    
    int arr[5] = {1, 2, 3, 4, 5};
    // 定义数组指针类型，注意 * 在数组指针类型名前面
    typedef int (*PMYARR)[5];
    // 根据数组指针类型定义数组指针变量
    // () 与 [] 优先级相同，但是结合律是从 左向右，所以：PMYARR 首先与 * 结合，此时PMYARR 是一个指针，然后与 []  结合，说明该指针指向一个数组，且该数组中有5个 int元素。
